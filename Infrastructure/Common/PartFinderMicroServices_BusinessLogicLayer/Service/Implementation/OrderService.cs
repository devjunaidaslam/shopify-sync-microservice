...
namespace PartFinderMicroServices_BusinessLogicLayer.Service.Implementation
{
    public class OrderService : IOrderService
    {
        ...
        private async Task PublishOrderToPredikoInternalAsync(
            PartFinderMicroServices_DataAccessLayer.Entities.DTOs.OrderDTO.OrderDTO orderDto,
            ...
            IPredikoOrderRMQService predikoRMQ)
        {
            ...
            try
            {
                ...
                var enrichmentLookup = new Dictionary<long, (PartFinderMicroServices_DataAccessLayer.Entities.DTOs.PredikoDTO.PredikoProductInfoDTO? Product, 
                                                              PartFinderMicroServices_DataAccessLayer.Entities.DTOs.PredikoDTO.PredikoVariantInfoDTO? Variant)>();

                // First, gather enrichment data from OrderLineItems
                foreach (var lineItem in orderDto.LineItems)
                {
                    PartFinderMicroServices_DataAccessLayer.Entities.DTOs.PredikoDTO.PredikoProductInfoDTO? productInfo = null;
                    PartFinderMicroServices_DataAccessLayer.Entities.DTOs.PredikoDTO.PredikoVariantInfoDTO? variantInfo = null;
                    ...
                    if (lineItem.VariantId.HasValue)
                    {
                        try
                        {
                            ...
                            if (variant != null)
                            {
                                var variantDto = new PartFinderMicroServices_DataAccessLayer.Entities.DTOs.PredikoDTO.PredikoVariantInfoDTO
                                {
                                ...
                                if (!string.IsNullOrWhiteSpace(variant.SKU))
                                {
                                    ...
                                    if (skuParts.Length > 0)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            if (supplier != null)
                                            {
                                                variantDto.Suppliers.Add(new PartFinderMicroServices_DataAccessLayer.Entities.DTOs.PredikoDTO.PredikoSupplierInfoDTO
                                                {
                    ...
                    if (lineItem.ProductId.HasValue)
                    {
                        try
                        {
                            ...
                            if (product != null)
                            {
                                ...
                                productInfo = new PartFinderMicroServices_DataAccessLayer.Entities.DTOs.PredikoDTO.PredikoProductInfoDTO
                                {
                ...
                foreach (var fulfillmentOrder in orderDto.FulfillmentOrders ?? new List<PartFinderMicroServices_DataAccessLayer.Entities.DTOs.OrderDTO.FulfillmentOrderDTO>())
                {
                    foreach (var foLineItem in fulfillmentOrder.LineItems ?? new List<PartFinderMicroServices_DataAccessLayer.Entities.DTOs.OrderDTO.FulfillmentOrderLineItemDTO>())
                    {
                ...
                var predikoMessage = new PartFinderMicroServices_DataAccessLayer.Entities.DTOs.PredikoDTO.PredikoOrderMessageDTO
                {
                    Order = orderDto,
